  ...
  $.ajaxSetup({
    ...
    jQuery.ajax({
        ...
        success: function(result){
          ...
          $('.user').html('<p>Estado: <small>'+result.user.name+'</small></p>');
  ...
  $.ajaxSetup({
    ...
    jQuery.ajax({
        ...
        $('#modalEdit').modal('show');
            ...
            $('.id').val(id);
            $('.text').html('<label>Descripción</label><textarea id="textarea" name="descripcion" class="textarea" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">'+result.noticia.descripcion+'</textarea>');
            $('.select-estado').empty();
...
jQuery('.btn-delete').click(function(e){
  $.ajaxSetup({
    ...
    jQuery.ajax({
        url: "/noticiasadmin/"+$(".btn-delete").val(),
        method: 'delete',
        success: function(result){
